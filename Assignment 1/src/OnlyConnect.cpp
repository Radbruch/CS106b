...
string onlyConnectize(string phrase) {
    ...
    if (islower(firstchar)) {
        string result(1, toupper(firstchar));
        phrase = phrase.substr(1);
        return  result + onlyConnectize(phrase);
    }
    if (isupper(firstchar)) {
        string result(1, firstchar);
        phrase = phrase.substr(1);
        return result + onlyConnectize(phrase);
    }
    else {
...
PROVIDED_TEST("Test string contains non-letter") {
    EXPECT_EQUAL(onlyConnectize(" love le2tte5r."), "LVLTTR");
    EXPECT_EQUAL(onlyConnectize(" 4i0~ ."), "");
}
