...
string reverseOfOrder(string s) {
    string reverseStr = "";
    for (char c: s) {
        if (c == 'A') {
            reverseStr += "B";
...
void isvalid(int n) {
    if (n < 0) {error("Can not pass in negative number");}
}
...
string aSequenceOfOrder(int n) {
    ...
    if (n == 0) return "A";
        ...
        return aSequenceOfOrder(n-1) + reverseOfOrder(aSequenceOfOrder(n-1));
    }
}
...
string bSequenceOfOrder(int n) {
    ...
    if (n == 0) return "B";
        ...
        return bSequenceOfOrder(n-1) + reverseOfOrder(bSequenceOfOrder(n-1));
    }
}
