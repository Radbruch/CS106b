...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
     ...
     */
    if (kGramLength <= 0) {
        error("kGramLength must be positive integer");
    }
        ...
        for (int i = 0; i <= str.length()-kGramLength; i++) {
            string gram = "";
            gram += str.substr(i, kGramLength);
            if (kGram.containsKey(gram)) {
                kGram.put(gram, kGram.get(gram) + 1);
            } else {
                kGram.put(gram, 1);
            }
        }
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    if (input.isEmpty()) error("input can not be empty.");
    else {
        for (double value: input.values()) {
            if (value == 0) {
                ...
                squaresum = squaresum + std::pow(value,2);
            }
        }

        if (numberofzero == input.size()) error("input can not all be zero.");
        else {
            squaresum = std::sqrt(squaresum);
            for (string key: input) {
                output.put(key, input.get(key) / squaresum);
            }
            return output;
        }
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    else if (numToKeep < 0) error("numToKeep can not be negative.");
    else if (numToKeep == 0) return {};
    else {
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    for (string key: lhs.keys()) {
        if (rhs.containsKey(key)) {
            cosinesimilarity += lhs.get(key) * rhs.get(key);
        }
    }
...
string guessLanguageOf(const Map<string, double>& textProfile,
     ...
     * this function.
     */
    double themostcosinesimilarity = 0;
    string guesslanguage;
    if (corpora.isEmpty()) error("corpora can not be empty.");
    else {
        for (Corpus language: corpora) {
            double cosinesimilarity = cosineSimilarityOf(textProfile, language.profile);
            if (cosinesimilarity > themostcosinesimilarity) {
                themostcosinesimilarity = cosinesimilarity;
                guesslanguage = language.name;
            }
        }
